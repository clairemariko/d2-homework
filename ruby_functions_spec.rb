...
class Functions_Practice < MiniTest::Test
  ...
  def test_volume_of_cube(length1)
    length1=2
    volume_corret= lenth1**3
    assert_equal(volume_correct, volume_test)

  end

  # #Given the radius of a sphere calculate the volume
  def test_volume_of_sphere(radius1)
    radius1=5
    volume_correct= 4/3*3.14*radius1**1
    assert_equal(volume_correct,volume_test)
  end
  ...
  def test_days_until_christmas(date=DateTime.Now)
